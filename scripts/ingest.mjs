...
import { readdirSync, existsSync, mkdirSync, writeFileSync, statSync, unlinkSync, readFileSync } from 'fs'
...
// --- Extract aspect ratios from WebP headers ---
function parseWebpDimensions(filepath) {
  try {
    const fd = readFileSync(filepath, { flag: 'r' })
    if (fd.length < 30) return null
    const riff = fd.toString('ascii', 0, 4)
    if (riff !== 'RIFF') return null
    const chunk = fd.toString('ascii', 12, 16)
    if (chunk === 'VP8 ') {
      const w = fd.readUInt16LE(26) & 0x3FFF
      const h = fd.readUInt16LE(28) & 0x3FFF
      return w && h ? { w, h } : null
    }
    if (chunk === 'VP8L') {
      const bits = fd.readUInt32LE(21)
      return { w: (bits & 0x3FFF) + 1, h: ((bits >> 14) & 0x3FFF) + 1 }
    }
    if (chunk === 'VP8X') {
      const w = (fd[24] | (fd[25] << 8) | (fd[26] << 16)) + 1
      const h = (fd[27] | (fd[28] << 8) | (fd[29] << 16)) + 1
      return { w, h }
    }
  } catch {}
  return null
...
for (const p of allProjects) {
  let webpPath
  if (p.type === 'image') {
    webpPath = join(ROOT, 'public', p.src)
  } else {
    // Use thumbnail for video aspect ratio
    const thumbPath = join(THUMBS_DIR, `${p.id}.webp`)
    webpPath = existsSync(thumbPath) ? thumbPath : null
  }
  if (webpPath && existsSync(webpPath)) {
    const dims = parseWebpDimensions(webpPath)
    if (dims) p.aspectRatio = Math.round((dims.w / dims.h) * 1000) / 1000
  }
}
...
const sized = allProjects.map(project => {
  if (project.multiFrame) {
    return { ...project, width: CLUSTER_W, height: CLUSTER_H }
  }
...
for (const item of shuffled) {
  ...
  const maxW = item.multiFrame ? CANVAS_W + CLUSTER_W : CANVAS_W
  const valid = candidates
    .filter(c => c.x + item.width <= maxW && c.x >= MARGIN && c.y >= MARGIN)
